...
# 재추천 요청
@broker.subscriber(service.get_recommendation_refresh_request_topic())
...
# 확정 요청
# 이벤트 타입 수정 필요
@broker.subscriber(service.get_restaurant_confirmed_topic())
async def handle_restaurant_confirmed(event, message = Context()):
    print("get restaurant confirmed request")
    print(event)

# 페르소나 요청
@broker.subscriber(service.get_user_persona_update_topic())
...
# OCR 요청
# 이벤트 타입 수정 필요
@broker.subscriber(service.get_receipt_ocr_request_topic())
